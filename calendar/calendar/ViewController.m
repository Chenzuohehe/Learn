...
//    NSDate *dateCheck = [dateFormatter dateFromString:stringDate];
    ...
    NSString *nowDate = [formatter stringFromDate:myDate];
    
    NSString *calenderDate = [formatter stringFromDate:date];
    
    NSLog(@"date:%@ caldendar:%@",nowDate ,calenderDate);
    
    if ([nowDate isEqual:calenderDate]) {
        return @"今天";
    }
    
    if (self.selectDate != nil) {
        NSString * selectDay = [formatter stringFromDate:self.selectDate];
        if ([selectDay isEqual:calenderDate]) {
            return @"发货";
        }
    }
    return nil;
...
//    NSString *nowDate = [formatter stringFromDate:myDate];
//    NSString *calenderDate = [formatter stringFromDate:date];
//    NSLog(@" nowDate%@,calendar: %@",nowDate,calenderDate);
//    if ([calenderDate isEqualToString:@"周六"] || [calenderDate isEqualToString:@"周日"]) {
